            ...
            await this.axios.get(KostalRequest2, { transformResponse: (r) => r })
                ...
                if (this.config.readanalogs) {
                    this.setStateAsync('Inputs.Analog1', {
                        val: (Math.round(100 *
                            (result[12].value / 10 * (this.config.normAn1Max - this.config.normAn1Min) + this.config.normAn1Min))) / 100,
                        ack: true
                    });
                    this.setStateAsync('Inputs.Analog2', {
                        val: (Math.round(100 *
                            (result[13].value / 10 * (this.config.normAn2Max - this.config.normAn2Min) + this.config.normAn2Min))) / 100,
                        ack: true
                    });
                    this.setStateAsync('Inputs.Analog3', {
                        val: (Math.round(100 *
                            (result[14].value / 10 * (this.config.normAn3Max - this.config.normAn3Min) + this.config.normAn3Min))) / 100,
                        ack: true
                    });
                    this.setStateAsync('Inputs.Analog4', {
                        val: (Math.round(100 *
                            (result[15].value / 10 * (this.config.normAn4Max - this.config.normAn4Min) + this.config.normAn4Min))) / 100,
                        ack: true
                    });
                }
            })
