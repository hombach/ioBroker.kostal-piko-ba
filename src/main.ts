...
var InverterType: string = 'unknown';       // Inverter type
var InverterAPIPiko: boolean   = false;     // Inverter API of Piko or Piko-BA inverters; Kostal Piko 6.0BA, 8.0BA, 10.0BA, 3.0, 5.5, 7.0, 10, 12, 15, 17, 20
var InverterAPIPikoMP: boolean = false;     // Inverter API of Piko MP inverters; Kostal PIKO 3.0-1 MP plus
var InverterUIVersion: string  = 'unknown'; // Inverter UI Version
var KostalRequestOnce: string  = '';        // IP request-string for one time request of system type etc.
var KostalRequest1: string     = '';        // IP request-string 1 for Pico live data
var KostalRequest2: string     = '';        // IP request-string 2 for Pico live data
var KostalRequestDay: string   = '';        // IP request-string for PicoBA daily statistics
var KostalRequestTotal: string = '';        // IP request-string for PicoBA total statistics
...
class KostalPikoBA extends utils.Adapter {
    adapterIntervals: any[]; // intervallList: ioBroker.Interval[]; - - ERROR not working with adapter-core 3.x; has to be any
    axios: Axios;
    xml2js: XML2JS;
    /*****************************************************************************************/
    ...
    private async onReady(): Promise<void> {
        ...
        if (this.config.ipaddress) {
            ...
            const totalIntervall = this.setInterval(() => {
                try {
                    this.ReadPikoTotal();
                } catch (error: any) {
                    this.log.warn(`Error while pulling total statistics from inverter: ${error}`);
                }
            }, this.config.polltimetotal);
            this.adapterIntervals.push(totalIntervall);
          
            await resolveAfterXSeconds(3);

            await this.ReadPikoDaily();
            const dailyIntervall = this.setInterval(() => {
                try {
                    this.ReadPikoDaily();
                } catch (error: any) {
                    this.log.warn(`Error while pulling daily statistics from inverter: ${error}`);
                }
    ...
    private onUnload(callback: () => void): void {
        try {
            ...
            for (const index in this.adapterIntervals) {
                this.clearInterval(this.adapterIntervals[index]);
            }
    ...
    * ReadPikoOnce *************************************************************************/
    ...
    /****************************************************************************************
    * ReadPiko *****************************************************************************/
    private async ReadPiko(): Promise<void> {
        const xml2js = require('xml2js');
    ...
    /****************************************************************************************
    * ReadPiko2 ****************************************************************************/
    private async ReadPiko2(): Promise<void> {
    ...
    /****************************************************************************************
    * ReadPikoDaily ************************************************************************/
    private async ReadPikoDaily(): Promise<void> {
    ...
    /****************************************************************************************
    * ReadPikoTotal ************************************************************************/
    private async ReadPikoTotal(): Promise<void> {
        const xml2js = require('xml2js');
    ...
    /***************************************************************************************/
    private async HandleConnectionError(stError: any, sOccasion: string, sErrorOccInt: string): Promise<void> {
